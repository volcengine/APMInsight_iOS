...
static NSString *const kUserNetworkErrorPlaceholder = @"https://www.baiiiidu.com";
    ...
    if (!_items) {
        ...
        void(^userExceptionBlock)(void) = ^{
            dispatch_async(dispatch_get_main_queue(), ^{
                ...
                if (strongSelf) {
                    ...
                    NSInteger exceptionCountsAfterRecord = kExceptionCounts + 5;
                    for (NSInteger i = kExceptionCounts; i < exceptionCountsAfterRecord; i++) {
                        [strongSelf recordUserException:[NSString stringWithFormat:@"ExceptionTypeDemoTest%ld", i] customs:@{@"customKeyDemoTest":@"customValueDemoTest"} filters:@{@"filterKeyDemoTest":@"filterValueDemooTest"} appID:kDefaultAppID callback:^(NSError * _Nullable error) {
                            if (error) {
                                success = NO;
                            }
                        }];
        ...
        void(^manualUserExceptionBlock)(void) = ^{
            dispatch_async(dispatch_get_main_queue(), ^{
                __strong typeof(self) strongSelf = weakSelf;
                if (strongSelf) {
                    UIAlertController *alert = [strongSelf userExceptionAlertWithOKHandler:^(NSString *exceptionType, NSString *customKey, NSString *customValue, NSString *filterKey, NSString *filterValue, NSString *appID) {
                        [strongSelf recordUserException:exceptionType customs:@{customKey:customValue} filters:@{filterKey:filterValue} appID:appID callback:^(NSError * _Nullable error) {
                                dispatch_async(dispatch_get_main_queue(), ^{
                                    if (error) {
        ...
        void(^networkErrorBlock)(void) = ^{
            __strong typeof(self) strongSelf = weakSelf;
            if (strongSelf) {
                NSString __block *urlString = @"https://www.baiiiidu.com";
